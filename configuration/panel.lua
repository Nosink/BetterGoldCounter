...
local bup = builder:CreateButton("", function()
    local x, y = ns.settings.GetPosition()
    local newY = y + 1
    ns.settings.SetPosition(x, newY)
    BGCBus:TriggerEvent(name .. "_SETTINGS_CHANGED", "xy")
end, "^", { width = 24, height = 24 }, lockedCB, 128)
local bdown = builder:CreateButton("", function()
    local x, y = ns.settings.GetPosition()
    local newY = y - 1
    ns.settings.SetPosition(x, newY)
    BGCBus:TriggerEvent(name .. "_SETTINGS_CHANGED", "xy")
end, "v", { width = 24, height = 24 }, bup)
local bleft = builder:CreateButton("", function()
    local x, y = ns.settings.GetPosition()
    local newX = x - 1
    ns.settings.SetPosition(newX, y)
    BGCBus:TriggerEvent(name .. "_SETTINGS_CHANGED", "xy")
end, "<", { width = 24, height = 24 }, bdown)
builder:CreateButton("", function()
    local x, y = ns.settings.GetPosition()
    local newX = x + 1
    ns.settings.SetPosition(newX, y)
    BGCBus:TriggerEvent(name .. "_SETTINGS_CHANGED", "xy")
end, ">", { width = 24, height = 24 }, bleft)

builder:CreateSection(L["LKEY_OPTIONS_BACKDROP_TITLE"], lockedCB)
